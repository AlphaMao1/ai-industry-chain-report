// §1 时间线事件墙（宿主 #chart-timeline）
// 年份色带垫底（2023–2026），牌匾贪心分层错落，kind=ink/brk/cur 着色，
// 连杆落轴，错峰 draw-in，点击任意牌匾下钻。数据全部来自 RPT.timeline。
(() => {
  const host = document.getElementById("chart-timeline");
  if (!host || !window.RPT) return;
  const body = U.frame(host, {
    title: "2023 → 2026 · 事件墙",
    sub: "底纹 = 年份 · 墨 = 行业事件 · 红 = 断裂/管制 · 蓝 = 当前周期 · 点击任意牌匾下钻",
    src: "公司披露 · 行业官方 · 券商研究",
  });
  ...
  function build(final) {
    body.innerHTML = "";
    const W = Math.min(1150, Math.max(620, body.getBoundingClientRect().width - 4));
    const xOf = yr => ML + (yr - X0) / (X1 - X0) * (W - ML - MR);

    // 贪心分层：按时间升序，放进不叠压的最低层
    const ps = PLAQUES.map(p => ({ ...p, w: Math.max(120, Math.ceil(nameW(p.label)) + 22) }))
    ...
    const svg = svgEl("svg", { width: "100%", height: H, viewBox: `0 0 ${W} ${H}`, style: "display:block", "data-drill-keep": "1" });
    ...
    [2023, 2024, 2025, 2026].forEach((yr, i) => {
      ...
      const bg = svgEl("rect", { x: x0, y: TOP - 8, height: axisY - TOP + 8, rx: 2,
      ...
      const seg = svgEl("rect", { x: x0, y: axisY + 7, height: 5, rx: 1,
    ...
    ps.forEach((p, k) => {
      ...
      const nm = svgEl("text", { x: p.x + 10, y: py + 27, style: `font:700 12px ${SERIF};fill:${c}` });
      nm.textContent = p.label.length > 13 ? p.label.slice(0, 13) : p.label;
      const nm2 = svgEl("text", { x: p.x + 10, y: py + 41, style: `font:700 12px ${SERIF};fill:${c}` });
      nm2.textContent = p.label.length > 13 ? p.label.slice(13) : "";
      ...
      g.addEventListener("click", e => {
        U.showDrill({ title: p.drill.title, value: p.drill.value, sub: p.drill.sub,
